...
const getRetailersMyDurians = async () => {
  ...
  for (let i = 0; i < data.length; i++) {
    const obj = data[i];
    if (
      obj.retailInfo.retailerAddress.toLowerCase() === currentAd.toLowerCase()
    ) {
  ...
  await durians.forEach((durian) => {
    ...
    const durianHTML = `
        <tr>
          ...
          <td class="align-left text-center text-sm">
            <p class="text-xs mb-0 prod-pText state-text">${dState}</p>
          </td>
...
addPriceButtons.forEach((button) => {
  button.addEventListener("click", function () {
    ...
    durians.forEach((item) => {
      if (item.id == durianId) durian = { ...item };
    });
...
submitButton.addEventListener("click", function () {
  ...
  if (price <= 0) {
    ...
    alert(`The durian unit price you entered is: ${price} ETH`);
    ...
    durianRow.forEach((row) => {
      ...
      if (durianName == form_durianId.value) {
        ...
        row
          ...
          .querySelector(
            ".price-text"
          ).innerHTML = `<p class="text-xs mb-0 prod-pText">${price} ETH</p>`;
